...
class BookService:
    ...
    def delete_book(self, id: str) -> None:
        ...
        try:
            ...
            # Check for active loans (not returned) - BY BOOK ID
            book_loans = [loan for loan in loan_service.get_all_loans() 
                         if loan.get_book_id() == id and not loan.is_returned()]
            
            if book_loans:
                loan_ids = [loan.get_loan_id() for loan in book_loans]
                raise ValueError(
                    f"Cannot delete book: Book ID '{id}' has {len(book_loans)} "
            ...
            historical_loans = [loan for loan in loan_service.get_all_loans() 
                               if loan.get_book_id() == id]
            
            if historical_loans:
                logger.warning(
                    f"Book {id} has {len(historical_loans)} "
    ...
    def clone_book(self, existing_id: str) -> Book:
        """Create a duplicate of an existing book with a new unique id.

        The cloned book will have all the same data fields as the original
        except for the `id` which is generated by `generate_next_id()` and
        `isBorrowed` which is always set to False (available by default).
        
        This method handles:
        1. Book catalog update (books.json)
        2. Inventory synchronization (inventory_general.json)
        
        Returns the newly created Book instance on success.
        Raises ValueError if the source book cannot be found.
        Raises ValueError if inventory synchronization fails.
        """
        src = self.find_by_id(existing_id)
        if src is None:
            raise ValueError(f"No book found with id '{existing_id}' to clone")

        # Generate a new unique id and construct a Book copy
        # IMPORTANT: isBorrowed is always False for new/cloned books (available by default)
        ...
        new_book = Book(
            ...
            src.get_weight(),
            src.get_price(),
            False,  # Always available when cloned
        )

        # Reuse add_book (validation + persistence to books.json)
        self.add_book(new_book)
        
        # Synchronize with inventory - CRITICAL for stock management
        try:
            from services.inventory_service import InventoryService
            inv_service = InventoryService()
            inv_service.add_item(new_book, 1)
            logger.info(f"Inventory synchronized for cloned book: id={new_book.get_id()}, ISBN={new_book.get_ISBNCode()}")
        except Exception as e:
            logger.error(f"Failed to synchronize inventory for cloned book {new_book.get_id()}: {e}")
            # Rollback: remove the book from catalog since inventory sync failed
            try:
                self.books = [b for b in self.books if b.get_id() != new_id]
                self._save_books()
                logger.info(f"Rolled back book {new_id} from catalog due to inventory sync failure")
            except Exception as rollback_error:
                logger.error(f"Rollback failed: {rollback_error}")
            raise ValueError(f"Book cloning failed: Could not synchronize inventory - {e}")
