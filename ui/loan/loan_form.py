"""
Loan Form Window Module

This module provides a graphical user interface for creating new loan records
in the library management system. It allows users to select a user and an
available book to create a new loan transaction.
"""
...
class LoanForm(ctk.CTkToplevel):
    """
    A top-level window for creating new loan records.

    This class provides a graphical interface that allows users to create new loans
    by selecting a user from the system and an available book from the inventory.
    The loan ID is automatically generated by the system. Only books with available
    copies (stock > 0) are displayed for selection.

    Attributes:
        _parent_window: Reference to the parent window that opened this dialog
        controller (LoanController): The loan controller instance for performing database operations
        _user_map (dict): Maps user display strings ("Name (ID)") to user IDs
        _book_map (dict): Maps book display strings ("Title (ISBN) [BookID]") to ISBNs
        user_selector (CTkOptionMenu or CTkLabel): Widget for selecting the user,
                                                    displays label if no users available
        book_selector (CTkOptionMenu or CTkLabel): Widget for selecting the book,
                                                    displays label if no books available
        btn_create (CTkButton): Primary button for creating the loan
    """

    def __init__(self, parent=None):
        """
        Initialize the loan creation form window.

        Sets up the window layout, loads available users and books from the database,
        and populates the selection widgets. Applies the application theme and
        configures window properties for proper display and behavior.
        ...
        Side Effects:
            ...
            - Loads all books with available copies from InventoryService
    ...
    def create_loan(self):
        """
        Handle the create loan button click event.

        Validates the form data, extracts the selected user ID and book ISBN,
        then calls the controller to create a new loan in the database. The loan ID
        is automatically generated by the system. Displays success or error messages
        ...
        Side Effects:
            - Shows error message box if validation fails or no users/books are available
            - Shows success message box if the loan is created successfully, including
              the generated loan ID when available
            - Closes the window after successful loan creation
            - Creates a new loan record in the database via the controller
            - Decrements the available stock for the selected book

        Raises:
            Exception: Catches and handles exceptions during user/book selection mapping
        """
    ...
    def _on_close(self):
        ...
        Properly closes the loan form window and returns focus to the parent window.
